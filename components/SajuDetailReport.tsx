...
export default function SajuDetailReport({ label, birthdate, birthTime, gender }: SajuDetailReportProps) {
  ...
  // 십신·격국·용신 같은 전문어는 화면 표면에 그대로 노출하지 않고, 펼치기 전엔
  // 복실이 말투 한두 줄 요약만 보여준다(여우.md v2의 톤 규칙).
  const summary = dayMasterEntry ? dayMasterEntry.temperament.split(".")[0] + "." : null;
  ...
  return (
    <div className="mt-4 rounded-xl bg-white p-4 text-left shadow-sm ring-1 ring-brown/10">
      ...
      {summary && <p className="mt-2 text-sm leading-relaxed text-brown-soft">{summary}</p>}

      <button
        type="button"
        onClick={() => setExpanded((v) => !v)}
        className="mt-2 text-xs font-bold text-coral-dark underline underline-offset-2"
      >
        {expanded ? "접기" : "🔍 상세 펼치기"}
      </button>
